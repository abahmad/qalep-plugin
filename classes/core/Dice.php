    ...
    class Dice {
        ...
        public function getRule($name) {
            ...
            foreach ($this->rules as $key => $rule) {
                if (empty($rule['instanceOf']) && $key !== '*' && is_subclass_of($name, $key) && (empty($rule['inherit']) || $rule['inherit'] === true ))
                    return $rule;
            }
        ...
        private function getClosure($name, array $rule) {
            ...
            return isset($rule['call']) ? function (array $args, array $share) use ($closure, $class, $rule) {
                ...
                foreach ($rule['call'] as $call)
                    call_user_func_array([$object, $call[0]], $this->getParams($class->getMethod($call[0]), $rule)->__invoke($this->expand($call[1])));
                return $object;
            } : $closure;
        }
        ...
        private function getParams(\ReflectionMethod $method, array $rule) {
            ...
            return function (array $args, array $share = []) use ($paramInfo, $rule) {
                ...
                if ($share || isset($rule['constructParams']))
                    $args = array_merge($args, isset($rule['constructParams']) ? $this->expand($rule['constructParams']) : [], $share);
                $parameters = [];
                ...
                foreach ($paramInfo as $p) {
                    ...
                    if ($class)
                        $parameters[] = $sub ? $this->expand($rule['substitutions'][$class], $share) : $this->create($class, [], $share);
                    else if ($args)
